...
@icon("../../sprites/interpolated_camera_3d.svg")
extends Camera3D
...
func _ready():
	EventBus.subscribe(GlobalEvent.CAMERA_SET_TARGET_CHARACTER, self, "_on_set_target_character")

func _on_set_target_character(event_data):
	target = event_data.get_node("Pivot/TargetCamera")
